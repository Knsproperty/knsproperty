  ...
  return (
    <Link href={`/property/${type}/${slug}`}>
      <div
        ...
        className=" h-full group rounded-md shadow-md md:hover:shadow-lg transition duration-500 md:hover:border-[tomato] border-transparent border-2 ease-in-out md:hover:-translate-y-5 overflow-hidden bg-white m-2"
      >
        <header className="overflow-hidden bg-lightgray ">
          <Slider {...settings} className="h-full">
            {image.map((url) => (
              <div className="relative h-[200px]">
                <Image className="w-full" src={url} alt="property image" fill />
              </div>
            ))}
          </Slider>
        </header>
        <main>
          <div className="px-5">
            <h2 className="text-md text-primary font-semibold my-3">{title}</h2>
...
export function SampleNextArrow(props: any) {
  ...
  return (
    ...
    >
      <FiArrowRight
        size={12}
        className=" -mt-5 -ml-[50px] w-[40px] h-[40px] bg-white shadow-md p-2.5 z-[99999] absolute rounded-full"
      />
    </div>
  );
}
...
export function SamplePrevArrow(props: any) {
  ...
  return (
    ...
    >
      <FiArrowLeft
        size={12}
        className=" -mt-5 ml-[30px] w-[40px] h-[40px] bg-white shadow-md p-2.5 rounded-full"
      />
    </div>
  );
}
