...
const Exclusive: React.FC<Props> = async ({ type }) => {
    ...
    return (
        <Container>
            <div>
                <h2 className="text-xl  font-medium pl-5 mt-5 mb-3">Exclusive Listings</h2>
                <div className="2xl:max-w-[1320px] xl:max-w-[1320px] lg:max-w-[80vw] md:max-w-[80vw] max-w-[100vw] ">
                    <Slider>
                        {[...__meta, ...__meta].map(({ attributes }: any, index: number) => <BuyExclusiveCard
                            {...{
                                property_type: type,
...
const getExlusiveProperty = async (type: string) => {
    const res = await fetch(`${process.env.NEXT_PUBLIC_URL}/api/exclusive/${type}`, {
        cache: "force-cache",
    });
