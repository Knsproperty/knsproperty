            ...
            <div className=" grid gap-5 lg:flex">
                ...
                <section className="relative ">
                    <div onClick={handleContainerClick} className="bg-white shadow-md  h-[60px] px-3 pl-5 items-center flex  rounded-full gap-3 border  border-[#80808039]">
                        ...
                        <input ref={inputRef} placeholder="Community or Building" className="border-none outline-none xl:min-w-[400px] placeholder:text-light placeholder:text-sm flex-1" />
                        <button className="hidden lg:block" onClick={handleCloseClick}><FiX size={20} /></button>
                        <button onClick={hadleSearch} className="bg-primary text-white px-5 py-2.5 rounded-full capitalize text-sm hidden md:block">search</button>
                        <button onClick={hadleSearch} className="bg-primary text-white px-5 py-2.5 rounded-full capitalize text-sm block md:hidden"><FiArrowRight className=" stroke-white" /></button>
                    </div>
                </section>
...
const PropertyType = () => {
    ...
    const buttons = ['commercial', 'residental', 'offplan'].map((name) => <button onClick={() => { setActive(name) }} className={`text-white px-5 py-2.5 rounded-full md:text-sm text-[13px] capitalize ${active == name ? 'bg-primary ' : 'bg-transparent'}`} key={name}>{name}</button>)
    return (
