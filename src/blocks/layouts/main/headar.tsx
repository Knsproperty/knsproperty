        ...
        <div className="dropdown dropdown-hover ">
            ...
            <div tabIndex={0} className="dropdown-content z-[1]  menu  w-[250px]  pt-[25px]">
                <div className="flex flex-col bg-lightgray p-5 rounded-md shadow">
                    {options.map((link) => <Link href={link.href} className="text-sm font-light hover:underline capitalize py-2">{link.text}</Link>)}
                </div>
            </div>
        </div>
    )
            ...
            <div className="max-w-[1320px] mx-auto h-[70px] grid grid-cols-[1fr_2fr_1fr] ">
                ...
                <nav className="flex items-center justify-around ml-5">
                    {__meta.map((link) => {
                        if (link.options) {
                            return (
                                <Dropdown text={link.text} options={link.options} />
                            )
                        }
                        return (
                            <Link href={link.href} className="text-sm font-light hover:underline capitalize">{link.text}</Link>
                        )
                    })}
                </nav>
